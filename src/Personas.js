...
router.post('/insertusuario', (req, res) =>{
    const { Cod_Persona, Tipo_Usuario, usu_contra, fec_registro, Estado  } = req.body;
    const query = ` CALL INS_USUARIO (?, ?, ?, ?, ?);`;

    mysqlConnection.query(query, [ Cod_Persona, Tipo_Usuario, usu_contra, fec_registro, Estado], (err, rows, fields) => {
     if(!err){
...
router.put('/updateusuario', (req, res) =>{
    const { Cod_usuario,Tipo_Usuario,usu_contra,fec_registro,Estado} = req.body;
    const query = ` CALL UPT_USUARIOS ( ?, ?, ?, ?, ?);`;
    let a= mysqlConnection.query(query, [Cod_usuario,Tipo_Usuario,usu_contra,fec_registro,Estado], (err, rows, fields) => {
        if(!err){
        res.json({Status: 'Datos Actualizados'})
     }else{
        console.log(err);
